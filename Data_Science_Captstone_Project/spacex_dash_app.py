...
def get_scatter_chart(entered_site, payload_range):
    if entered_site == 'ALL':
        filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'].ge(int(payload_range[0]))) & 
                                (spacex_df['Payload Mass (kg)'].le(int(payload_range[1])))
                                ]
        ...
        filtered_df = spacex_df[(spacex_df['Launch Site'].eq(entered_site)) &
                                (spacex_df['Payload Mass (kg)'].ge(int(payload_range[0]))) & 
                                (spacex_df['Payload Mass (kg)'].le(int(payload_range[1])))
                                ]
